...
<?php
    ...
    if (isset($_POST['DNI'])) {
        // Obtener el DNI del profesor desde el POST
        $dni = $_POST['DNI'];

        // Consulta para verificar si el DNI ya existe en la base de datos
        $check_query = "SELECT * FROM profesor WHERE DNI_profesor = '$dni'";
...
?>
                <form method="POST" action="">
                    ...
                    <br>
                    <p>¿Estás seguro que quieres eliminar a este profesor?</p>
                    <input type="submit" name="eliminar" value="Eliminar Profesor" class="btn btn-danger" style="background-color: red;">
                </form>
                <div class="volvido">
...
<?php
            ...
            $mensaje = "<div class='alert alert-danger'>El profesor con DNI '$dni' no existe.</div>";
        }
    }

    // Código para eliminar al profesor
    if (isset($_POST['eliminar']) && isset($_POST['modiDNI'])) {
        $dni_eliminar = $_POST['modiDNI'];
        $query_delete = "DELETE FROM profesor WHERE DNI_profesor ='$dni_eliminar'";
        ...
        if ($resultado) {
            echo "<script>
                Swal.fire({
                    ...
                    html: '<a href=\"http://localhost:8080/escuela1/profesores/listarProfesor.php\" class=\"btn btn-success\">VOLVER A LISTA DE PROFESORES</a>',
